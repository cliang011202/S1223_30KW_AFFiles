...
class BsplineTwist(om.ExplicitComponent):
    ...
    def initialize(self):
        ...
        self.options.declare("n_span", types=int)
        self.options.declare("r", types=np.ndarray)
        self.options.declare("spline_order", default=4, types=int)
        self.options.declare("n_cyl", default=0, types=int,
                             desc="Number of cylinder sections at root (fixed twist)")
        self.options.declare("twist_cyl", default=np.array([]), 
                             desc="Fixed twist for cylinder sections (rad)")
    ...
    def setup(self):
        ...
        n_cyl  = self.options["n_cyl"]

        # 非圆柱段的截面数
        self.n_cyl = n_cyl
        self.n_aero = n_aero = n_span - n_cyl

        # 只对非圆柱段建立 B-spline
        r_aero = r_arr[n_cyl:]
        s_aero = (r_aero - r_aero[0]) / (r_aero[-1] - r_aero[0])
        self.s_aero = s_aero
        ...
        knots = np.concatenate([np.zeros(k), internal_knots, np.ones(k)])
        self.knots = knots
        self.k = k

        # 基函数矩阵: (n_aero, n_cp)
        self.B_matrix = np.zeros((n_aero, n_cp))
        for j in range(n_cp):
            c_j = np.zeros(n_cp)
            c_j[j] = 1.0
        ...
        # 完整 Jacobian: (n_span, n_cp)，圆柱段行全为 0
        self.B_full = np.zeros((n_span, n_cp))
        self.B_full[n_cyl:, :] = self.B_matrix

        self.add_input("twist_cp", val=np.zeros(n_cp), units="rad")
        self.add_output("theta_in", val=np.zeros(n_span), units="rad")

        # Jacobian: 圆柱段对 twist_cp 的导数为 0，非圆柱段为 B_matrix
        self.declare_partials("theta_in", "twist_cp", val=self.B_full)

    def compute(self, inputs, outputs):
        n_cyl = self.n_cyl
        twist_cyl = self.options["twist_cyl"]

        theta = np.zeros(self.options["n_span"])
        # 圆柱段: 固定值
        theta[:n_cyl] = twist_cyl
        # 非圆柱段: B-spline 插值
        theta[n_cyl:] = self.B_matrix @ inputs["twist_cp"]

        outputs["theta_in"] = theta

    def fit_initial_twist(self, twist_rad):
        """只拟合非圆柱段的扭角"""
        twist_aero = twist_rad[self.n_cyl:]
        cp, _, _, _ = np.linalg.lstsq(self.B_matrix, twist_aero, rcond=None)
        return cp
...
N_CP = 8  # B-spline 控制点数量 (可调: 5~8)
        ...
        # 圆柱段配置：前2个截面为圆柱段，扭角固定

N_CYL = 2  # 圆柱段截面数量
twist_cyl_rad = np.deg2rad(twist_init_deg[:N_CYL])  # 固定的圆柱段扭角
...
class TwistOptGroup(om.Group):
    ...
    def initialize(self):
        ...
        self.options.declare("n_cp", types=int)
        self.options.declare("r", types=np.ndarray)
        self.options.declare("n_cyl", default=0, types=int)          # 新增
        self.options.declare("twist_cyl", default=np.array([]))      # 新增
...
bspl_temp = BsplineTwist(
    n_cp=N_CP, n_span=n_span, r=r, spline_order=4,
    n_cyl=N_CYL, twist_cyl=twist_cyl_rad,   # 新增
)
...
# ---- 设计变量: B-spline 控制点 ----
# 上下界基于初始值 ± 15°（控制点空间）
cp_lower = cp_init - np.deg2rad(10.0)
cp_upper = cp_init + np.deg2rad(5.0)
